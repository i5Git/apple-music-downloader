...
func finalizeAppleMusicMP4(trackPath string) error {
	...
	if err != nil {
		return fmt.Errorf("MP4Box finalization output is unavailable: %w", err)
	}
	if info.Size() <= 0 {
		return errors.New("MP4Box finalization produced an empty file")
	}
	return nil
}
...
func downloadTrackSilently(track structs.TrackData, meta *structs.AutoGenerated, albumId, storefront, baseSaveFolder, Codec, covPath string, qobuzDesc string, lyricAccount *structs.Account, account *structs.Account, progressChan chan runv14.ProgressUpdate, jsonOutput bool, maxAlbumSampleRate float64, m3u8Cache *sync.Map) (string, error) {
	...
	// Always ask the wrapper/device for the real M3U8 in hires/all mode.
	...
	if err := finalizeAppleMusicMP4(tempTrackPath); err != nil {
		return "", fmt.Errorf("failed to finalize Apple Music M4A: %w", err)
	}
	...
	if core.Config.EmbedCover {
		if strings.Contains(albumId, "pl.") && core.Config.DlAlbumcoverForPlaylist {
			...
			var err error
			coverPath, err = metadata.WriteCover(finalAlbumFolder, strings.TrimSuffix(safeCoverFilename, ".jpg"), track.Attributes.Artwork.URL)
			if err != nil {
				coverPath = ""
			}
		} else {
	...
	fullTrackCount := len(meta.Data[0].Relationships.Tracks.Data)
	if err := metadata.WriteMP4Tags(tempTrackPath, finalLrc, qobuzDesc, meta, trackIndexInMeta, fullTrackCount, coverPath); err != nil {
		return "", fmt.Errorf("元数据写入失败，文件可能不完整: %w", err)
	}

	if strings.Contains(albumId, "pl.") && core.Config.DlAlbumcoverForPlaylist && coverPath != "" {
...
func RipStation(stationId string, storefront string, urlRaw string, jsonOutput bool) error {
	...
	if isLive || isRadioStation {
		...
		if err != nil {
			return fmt.Errorf("下载电台流失败: %v", err)
		}
