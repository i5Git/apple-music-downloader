...
anchor = s.index("// Select only a stream that actually matches the requested mode.")
start = s.index("\t} else if core.Dl_aac {\n", anchor)
end = s.index("\n\t} else {\n\t\tvar bestVariant *m3u8.Variant", start)

new = r'''	} else if core.Dl_aac {
...
s = s[:start] + new + s[end:]
p.write_text(s)
print("Applied AAC fallback ladder")
